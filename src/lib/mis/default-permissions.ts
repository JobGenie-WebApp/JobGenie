/**
 * Single source of truth for the default MIS permission catalog.
 *
 * Used by:
 *  - `scripts/seed-mis-permissions.ts` (manual seeding script)
 *  - `src/lib/bootstrap/seed-super-admin.ts` (startup auto-seed)
 *
 * Each entry maps to a row in `mis_permissions` (name = `${resource}.${action}`).
 */

export interface MisPermission {
  resource: string;
  action: string;
  description: string;
}

export const DEFAULT_PERMISSIONS: MisPermission[] = [
