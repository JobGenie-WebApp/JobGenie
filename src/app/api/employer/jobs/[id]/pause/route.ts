...
// POST /api/employer/jobs/[id]/pause
export async function POST(_request: NextRequest, { params }: Params) {
    try {
        ...
        await admin.from("jobs").update({ status: "paused" }).eq("id", id);
        return NextResponse.json({ success: true });
    } catch (error) {
        await logError({ source: "api/employer/jobs/[id]/pause:POST", errorType: "APIError", message: error instanceof Error ? error.message : String(error) });
