...
import {
    Bell, BellOff, Save, Loader2,
    FileText, CalendarCheck, CalendarClock, AlarmClock,
...
const NOTIFICATION_OPTIONS: { key: NotifKey; label: string; description: string; icon: React.ElementType }[] = [
    { key: "new_application_received",       label: "New Application Received",  description: "When a candidate applies to one of your job postings",          icon: FileText },
    { key: "interview_scheduled_employer",   label: "Interview Scheduled",        description: "When a new interview round is confirmed on your calendar",      icon: CalendarCheck },
    { key: "interview_rescheduled_employer", label: "Interview Rescheduled",      description: "When an existing interview time is changed or cancelled",       icon: CalendarClock },
    { key: "interview_reminder_employer",    label: "Interview Reminder",         description: "Advance reminders before your scheduled interviews",            icon: AlarmClock },
    { key: "invitation_accepted",            label: "Invitation Accepted",        description: "When a candidate accepts your job invitation",                  icon: UserCheck },
    { key: "invitation_declined",            label: "Invitation Declined",        description: "When a candidate declines your job invitation",                 icon: UserX },
    { key: "company_approval_status",        label: "Company Approval Updates",   description: "When MIS approves or rejects your company registration",        icon: ShieldCheck },
];
    ...
    useEffect(() => {
        fetch("/api/user/notification-preferences").then(r => r.json()).then(d => {
            if (d.success && d.preferences) {
                const m = { ...DEFAULT_PREFS, ...d.preferences } as PrefsMap;
                setPrefs(m); setSavedPrefs(m);
            }
